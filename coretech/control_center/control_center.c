...
/* time measurement */
#define CC_TIME_START(s) \
	if (cc_time_measure) \
		s = ktime_get();

#define CC_TIME_END(s, e, t, tmax)  \
	if (cc_time_measure) { \
		e = ktime_get(); \
		t = ktime_to_us(ktime_sub(e, s)); \
		if (t > tmax) \
			tmax = t; \
...
static int cc_dump_status_show(char *buf, const struct kernel_param *kp)
{
	...
	cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, "expected ddrfreq: %d\n", atomic_read(&cc_expect_ddrfreq));
	return cnt;
}
